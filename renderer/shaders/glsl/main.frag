...
layout(set = 1, binding = UF_SAMPLER_BINDING)
uniform sampler uf_sampler;
layout(set = 1, binding = UF_TEX_BASE_COLOR_BINDING)
uniform texture2D base_color[MAX_TEXTURE_COUNT];
layout(set = 1, binding = UF_TEX_METALLIC_ROUGHNESS_BINDING)
uniform texture2D metallic_roughness[MAX_TEXTURE_COUNT];
layout(set = 1, binding = UF_TEX_NORMAL_BINDING)
uniform texture2D normal[MAX_TEXTURE_COUNT];
layout(set = 1, binding = UF_TEX_OCCLUSION_BINDING)
uniform texture2D occlusion[MAX_TEXTURE_COUNT];
layout(set = 1, binding = UF_TEX_EMISSIVE_BINDING)
uniform texture2D emissive[MAX_TEXTURE_COUNT];
layout(set = 1, binding = UF_PBR_FACTORS_BINDING, std430)
uniform PbrFactorsSoa {
...
layout(set = 1, binding = UF_DRAW_CALL_PARAMS_BINDING, std430)
uniform DrawCallParametersSoa {
